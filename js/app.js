...
function selects_close(e) {
	...
	// const buton = document.querySelector('.error');
	...
	for (let index = 0; index < select_options.length; index++) {
		...
		select_option.addEventListener('click', function () {
			...
			for (let index = 0; index < select_options.length; index++) {
				const el = select_options[index]; 
					select_options[0].style.display = 'none';
		...
		function addError(){
			button.addEventListener('click', function(e){
				...
				if (select_options[0].innerText == span ){
					errorBlock.classList.add('_empty')
					selectArrowError = document.querySelector('.select__value')
					selectArrowError.classList.add('_error')
					console.log(selectForm);	
				};
				selectArrowError.addEventListener("click", function(e){
					selectArrowError.classList.remove('_error')
					errorBlock.classList.remove('_empty')
				})
			});
		};
